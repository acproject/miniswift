...
struct ErrorProtocol {
    ...
    virtual std::string getLocalizedDescription() const { return getDescription(); }
...
struct TypeError : public ErrorProtocol {
    ...
    TypeError(const std::string& expected, const std::string& actual, const std::string& ctx = "")
        : expectedType(expected), actualType(actual), context(ctx) {}
    
    std::string getDescription() const override {
        std::string desc = "Type Error: Expected " + expectedType + ", got " + actualType;
        if (!context.empty()) {
            desc += " in " + context;
        }
        return desc;
...
public:
    static bool shouldPropagate(const ErrorValue& /* error */, const ErrorContext& context) {
