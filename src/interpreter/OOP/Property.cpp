...
void PropertyValue::setValue(Interpreter& interpreter, const Value& newValue) {
    ...
    if (!definition_.isVar && isInitialized_) {
        throw RuntimeError("Cannot assign to value: '" + definition_.name.lexeme + "' is a 'let' constant");
    }
...
Value PropertyValue::executeAccessor(Interpreter& interpreter, AccessorType type, const Value* newValue) {
    const auto* accessor = findAccessor(type);
    if (!accessor) {
        if (type == AccessorType::SET) {
            throw RuntimeError("Cannot assign to value: '" + definition_.name.lexeme + "' is a read-only property");
        }
        return Value(); // 没有找到访问器，返回空值
    }
    
    // 创建特殊的访问器环境，能够处理属性赋值
    auto accessorEnv = std::make_shared<PropertyAccessorEnvironment>(environment_, container_, &interpreter);
    ...
    // 不再预先定义属性变量，让 PropertyAccessorEnvironment::get 直接从属性系统获取
    // 这样可以确保访问器中总是能获取到最新的属性值
...
const PropertyDefinition* PropertyManager::getProperty(const std::string& name) const {
...
InstancePropertyContainer::InstancePropertyContainer(const PropertyManager& manager, std::shared_ptr<Environment> env)
    : environment_(env) {
    // 为所有属性创建实例
...
// Copy constructor
InstancePropertyContainer::InstancePropertyContainer(const InstancePropertyContainer& other)
...
Value InstancePropertyContainer::getProperty(Interpreter& interpreter, const std::string& name) {
...
void InstancePropertyContainer::setProperty(Interpreter& interpreter, const std::string& name, const Value& value) {
    auto it = properties_.find(name);
    if (it == properties_.end()) {
        throw RuntimeError("Undefined property '" + name + "'");
    }
...
const PropertyDefinition* InstancePropertyContainer::getPropertyDefinition(const std::string& name) const {
    auto it = properties_.find(name);
    if (it != properties_.end()) {
...
void InstancePropertyContainer::initializeDefaults(Interpreter& interpreter) {
    for (auto& [name, propValue] : properties_) {
        ...
        if (def.propertyType == PropertyType::STORED && def.defaultValue && !propValue->isInitialized()) {
...
void PropertyAccessorEnvironment::assign(const Token& name, const Value& value) {
    // 检查是否是属性赋值
