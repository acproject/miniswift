...
private:
  void checkBounds(size_t index) const {
    if (index >= data.size()) {
      throw ArrayBoundsException("Index " + std::to_string(index) +
...
public:
  ...
  // 内存区域管理
  void registerMemoryRegion(void *start, size_t size,
  ...
  // 数组边界检查
  void checkArrayBounds(void *array, size_t index, size_t size,
                        const std::string &debugInfo = "");
...
SafePtr<T> makeSafeNew(const std::string &debugInfo = "", Args &&...args) {
  ...
  MemorySafetyManager::getInstance().registerMemoryRegion(ptr, sizeof(T),
                                                          debugInfo);
  return SafePtr<T>(ptr, debugInfo);
}

template <typename T> void safeDelete(SafePtr<T> &ptr) {
  if (ptr) {
    T *rawPtr = ptr.get();
